module.exports = function (less) {
    ...
    var translationProperties = ['translate', 'translateX', 'translate3d'];
    var shortHandProperties = ["margin", "border-width", "padding", "border-radius", "border", "border-style", "border-color"];
    ...
    function separateDeclarationRules(declarationRules, visitArgs, visitor) {
        ...
        declarationRules.forEach(rule => {
            ...
            if (visitArgs.doRemove ||
                visitArgs.shortHandReorder ||
                visitArgs.reverseValues ||
                visitArgs.reverseKeywords ||
                visitArgs.specific ||
                visitArgs.negateTranslation ||
                visitArgs.negateBoxshadow) {
    ...
    AutoRTLPlugin.prototype = {
        ...
        visitDeclaration: function (ruleNode, visitArgs) {
            ...
            visitArgs.negateTranslation = false;
            visitArgs.negateBoxshadow = false;
            ...
            if (doReverse && nodeName.match(/(.*)(box-shadow)(.*)/)) {
                visitArgs.negateBoxshadow = true;
            }
        ...
        visitValue(valueNode, visitArgs) {
            if (visitArgs.shortHandReorder || visitArgs.negateBoxshadow) {
        ...
        visitExpression: function (expressionNode, visitArgs) {
            if (expressionNode.value.length == 1 && expressionNode.value[0].type == "Variable") {
                visitArgs.reverseKeywords = false;
            } else if (visitArgs.visitExpression &&
                visitArgs.shortHandReorder &&
                expressionNode.value.length == 4 &&
                expressionNode.value[1] != expressionNode.value[3]) {
            ...
            } else if (visitArgs.visitExpression && visitArgs.negateBoxshadow) {
                const copiedValues = [...expressionNode.value];
                if (isNumber(copiedValues[0].value)) {
                    copiedValues[0] = new less.tree.Negative(expressionNode.value[0]);
                    return new less.tree.Expression(copiedValues);
                }
        ...
        visitCall(callNode, visitArgs) {
            if (visitArgs.negateTranslation &&
                translationProperties.includes(callNode.name)) {
        ...
        visitAnonymous: function (valueNode, visitArgs) {
            ...
            } else if (visitArgs.negateBoxshadow && Number.isInteger(parseInt(values[0]))) {
                return new less.tree.Anonymous("-" + values[0] + (values.length > 1 ? " " : "") + values.splice(1, values.length).join(" "));
            }
            visitArgs.shortHandReorder = false;
